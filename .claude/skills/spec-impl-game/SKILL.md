...
name: spec-impl-game
description: >
  Implementa un spec aprobado de integración de juego en Arcade Vault y,
  al terminar, lanza skin-designer y luego mobile-porter secuencialmente
  para el juego recién implementado. Uso: /spec-impl-game <NN-spec-name>
argument-hint: <NN-spec-name>
---

# /spec-impl-game — Implementador de juegos con auditorías post-implementación

Igual que `/spec-impl` pero añade una Fase 5 que ejecuta `skin-designer`
y `mobile-porter` en orden secuencial al terminar la implementación.
...
Once the spec is marked as Implemented and the final commit is made, continue to Phase 5.

---

### Phase 5 — Post-implementation audits

This phase runs automatically after the final commit of Phase 4. Do not ask for confirmation — proceed immediately.

#### Step 5.1 — Extract game-id

Derive the game-id from the spec filename:

- Pattern: `NN-<game-id>-integration.md` → game-id = `<game-id>`
- Examples:
  - `08-snake-integration.md` → `snake`
  - `09-space-invaders-integration.md` → `space-invaders`
- If the filename does not match this pattern, extract the most likely game-id slug and confirm with the user before continuing.

Announce:

```
🎮 Implementation complete. Starting post-implementation audits for game: <game-id>

  [1/2] skin-designer — auditing and creating skins...
```

#### Step 5.2 — Run skin-designer

Invoke the `skin-designer` skill with the game-id as argument:

```
Skill tool: skill="skin-designer", args="<game-id>"
```

Wait for `skin-designer` to finish completely before proceeding. Do not run it in parallel with mobile-porter.

#### Step 5.3 — Run mobile-porter

Once `skin-designer` has finished, announce:

```
  [2/2] mobile-porter — auditing mobile experience...
```

Then invoke the `mobile-porter` skill with the game-id as argument:

```
Skill tool: skill="mobile-porter", args="<game-id>"
```

Wait for `mobile-porter` to finish completely.
...
```
✅ Game <game-id> fully integrated.

| Audit         | Result |
|---------------|--------|
| skin-designer | <summary from skin-designer> |
| mobile-porter | <summary from mobile-porter> |

Branch: spec-NN-slug
Next step: open a PR to merge into main.
```
...
```
/spec-impl-game 08-snake-integration

  Phase 1  →  Finds specs/08-snake-integration.md
  Phase 2  →  Reads the state → "Aprobado" → ✅ continues
  Phase 3  →  git checkout -b spec-08-snake-integration
              Shows objective, scope, plan and criteria
  Phase 4  →  Implements step by step with pauses
              Ends by marking spec as Implemented + final commit
  Phase 5  →  Extracts game-id = "snake"
              Runs skin-designer snake (waits to finish)
              Runs mobile-porter snake (waits to finish)
              Shows combined final report

/spec-impl-game 09-powerups  (state: Draft / Borrador)

  Phase 1  →  Finds specs/09-powerups.md
  Phase 2  →  Reads the state → "Draft" → ❌ stops
              Shows the standard error message
              Does not create branch, does not touch code, does not run audits
```
